#CREATED BY PALO ALTO NETWORKS PSC/CPSP IDEUS
import paramiko
import time
...
def disable_pager_and_repeat_show_counters(repetitions, interval, output_file):
    ...
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag clear all\n')
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag clear log log\n')
    time.sleep(1)  # Give it a second to process the command
    ...
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag set log feature flow basic\n')
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag set filter on\n')
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag set capture on\n')
    time.sleep(1)  # Give it a second to process the command
    ssh_shell.send('debug dataplane packet-diag set log on\n')
    time.sleep(1)  # Give it a second to process the command
    ...
    with open(output_file, 'w') as file:
        ...
        for i in range(repetitions):
            ...
            # Execute the additional commands after the last iteration
            if i + 1 == repetitions:
                ssh_shell.send('debug dataplane packet-diag set capture off\n')
                time.sleep(10)  # Give it a second to process the command
                ssh_shell.send('debug dataplane packet-diag aggregate-logs\n')
                time.sleep(10)  # Give it a second to process the command
                ssh_shell.send('show clock\n')
                time.sleep(1)  # Give it a second to process the command
                ssh_shell.send('exit\n')
